 ...
 *   TestProject.uproject  — minimal JSON pointing at UnrealAgent plugin
 *   Plugins/UnrealAgent/ — directory junction to the real plugin source
...
export function generateTempProject(): string {
  const dir = path.join(os.tmpdir(), `UnrealAgent_Test_${Date.now()}`);
  ...
  const uproject = {
    ...
    Plugins: [{ Name: "UnrealAgent", Enabled: true }],
  };
  fs.writeFileSync(
  ...
  // Plugins/UnrealAgent → junction to real plugin
  const pluginsDir = path.join(dir, "Plugins");
  fs.mkdirSync(pluginsDir, { recursive: true });

  const junctionTarget = path.join(pluginsDir, "UnrealAgent");
...
export function cleanupTempProject(): void {
  if (!tempDir) return;
  const junctionPath = path.join(tempDir, "Plugins", "UnrealAgent");
