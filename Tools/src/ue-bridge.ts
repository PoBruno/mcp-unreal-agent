...
export const PLUGIN_MODULE_NAME = "UnrealAgent";
...
export function findEditorCmd(): string | null {
  ...
  if (engineVersion) {
    ...
    for (const p of versionCandidates) {
      if (fs.existsSync(p)) {
        console.error(`[UnrealAgent] Auto-detected engine ${engineVersion} from .uproject`);
        return p;
      }
    }
  }
...
/**
 * Ensure the .modules file in Binaries/Win64/ lists the UnrealAgent module.
  ...
  try {
    ...
    if (data.Modules && !data.Modules[PLUGIN_MODULE_NAME]) {
      ...
      if (!fs.existsSync(dllPath)) {
        console.error(`[UnrealAgent] Warning: ${PLUGIN_DLL_NAME} not found — editor module may not be compiled.`);
        return;
      }
      data.Modules[PLUGIN_MODULE_NAME] = PLUGIN_DLL_NAME;
      fs.writeFileSync(modulesPath, JSON.stringify(data, null, "\t") + "\n", "utf-8");
      console.error(`[UnrealAgent] Fixed .modules file — added ${PLUGIN_MODULE_NAME} entry.`);
    }
  } catch (e) {
    console.error("[UnrealAgent] Warning: could not check/fix .modules file:", e);
  }
}
...
export async function gracefulShutdown(): Promise<void> {
  ...
  if (state.ueProcess) {
    ...
    if (!exited && state.ueProcess) {
      console.error("[UnrealAgent] Graceful shutdown timed out, force-killing.");
...
export async function spawnAndWait(): Promise<string | null> {
  ...
  const logPath = path.join(UE_PROJECT_DIR, "Saved", "Logs", "UnrealAgent_server.log");

  console.error("[UnrealAgent] Spawning UE5 commandlet...");
  ...
  state.ueProcess.on("exit", (code) => {
    console.error(`[UnrealAgent] UE5 server exited with code ${code}`);
    state.ueProcess = null;
  });
  ...
  console.error("[UnrealAgent] Waiting for health check (up to 3 min)...");
  const ok = await waitForHealthy(180);

  if (ok) {
    console.error("[UnrealAgent] UE5 Blueprint server is ready.");
    return null; // success
  }
  ...
  return "UE5 Blueprint server failed to start within 3 minutes. Check Saved/Logs/UnrealAgent_server.log.";
}
...
export async function ensureUE(): Promise<string | null> {
  ...
  if (state.ueProcess) {
    console.error("[UnrealAgent] UE5 process exists but is not healthy. Killing and respawning...");
    state.ueProcess.kill();
    state.ueProcess = null;
  }
