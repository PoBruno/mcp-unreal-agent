...
/**
 * FUnrealAgentServer — plain C++ class (not a UCLASS) that owns all HTTP
 * serving logic for the Blueprint MCP protocol.
 *
 * Both the standalone commandlet (UUnrealAgentCommandlet) and the in-editor
 * subsystem (UUnrealAgentEditorSubsystem) delegate to an instance of this
...
private:
	...
	// ----- Level / actor handlers (extracted into UnrealAgentHandlers_Level.cpp) -----
	...
	// ----- Level actor manipulation (UnrealAgentHandlers_LevelActors.cpp) -----
	...
	// ----- Level / actor helpers (UnrealAgentHandlers_Level.cpp) -----
