// UnrealAgentHandlers_Groom.cpp — Groom Binding asset tools
...
FString FUnrealAgentServer::HandleListGroomBindings(const TMap<FString, FString>& Params)
{
...
FString FUnrealAgentServer::HandleDuplicateGroomBinding(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent Groom: Duplicating '%s' -> '%s/%s'"),
...
FString FUnrealAgentServer::HandleSetGroomBindingTargetMesh(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	if (Json->HasField(TEXT("sourceMeshPath")))
	{
		...
		if (!SourceMeshPath.IsEmpty())
		{
			...
			if (SourceObjProp)
			{
				...
				{
					UE_LOG(LogTemp, Warning, TEXT("UnrealAgent Groom: sourceMeshPath '%s' could not be loaded — skipped"), *SourceMeshPath);
				}
			}
		}
	}
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent Groom: SetTargetMesh '%s' -> '%s', save %s"),
...
FString FUnrealAgentServer::HandleRebuildGroomBindings(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
