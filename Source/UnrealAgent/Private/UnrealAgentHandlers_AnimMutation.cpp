...
FString FUnrealAgentServer::HandleCreateAnimBlueprint(const FString& Body)
{
	...
	if (SkeletonName == TEXT("__create_test_skeleton__"))
	{
		...
		if (Skeleton)
		{
			...
			UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Created test skeleton '%s'"), *Skeleton->GetName());
		}
	}
	else
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Creating AnimBlueprint '%s' in '%s' with skeleton '%s'"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Created AnimBlueprint '%s' with %d graphs (saved: %s)"),
...
FString FUnrealAgentServer::HandleRemoveAnimState(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleAddAnimTransition(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleSetTransitionRule(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleAddAnimNode(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleAddStateMachine(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleSetStateAnimation(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleListAnimSlots(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleListSyncGroups(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleCreateBlendSpace(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Creating Blend Space '%s' in '%s' with skeleton '%s'"),
		*Name, *PackagePath, *Skeleton->GetName());
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Created Blend Space '%s' (saved: %s)"),
...
FString FUnrealAgentServer::HandleSetBlendSpaceSamples(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Set %d samples on Blend Space '%s' (saved: %s)"),
...
FString FUnrealAgentServer::HandleSetStateBlendSpace(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	auto WireVariable = [&](const FString& VarName, const FString& PinName) -> bool
	{
		...
		if (!bVarFound)
		{
			UE_LOG(LogTemp, Warning, TEXT("UnrealAgent: Variable '%s' not found in '%s', skipping wire"),
