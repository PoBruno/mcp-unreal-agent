...
FString FUnrealAgentServer::HandleAttachActor(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: attach_actor(child='%s', parent='%s', socket='%s')"), *ChildLabel, *ParentLabel, *SocketName);
...
FString FUnrealAgentServer::HandleDetachActor(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body."));
	FString ActorLabel;
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: detach_actor('%s')"), *ActorLabel);
...
FString FUnrealAgentServer::HandleDuplicateActor(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body."));
	FString ActorLabel;
	if (!Json->TryGetStringField(TEXT("actorLabel"), ActorLabel) || ActorLabel.IsEmpty())
		return MakeErrorJson(TEXT("Missing required field: 'actorLabel'."));
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: duplicate_actor('%s')"), *ActorLabel);
...
FString FUnrealAgentServer::HandleRenameActor(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body."));
	FString ActorLabel;
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: rename_actor('%s' -> '%s')"), *ActorLabel, *NewLabel);
