...
FString FUnrealAgentServer::GenerateSnapshotId(const FString& BlueprintName)
{
...
FGraphSnapshotData FUnrealAgentServer::CaptureGraphSnapshot(UEdGraph* Graph)
{
...
void FUnrealAgentServer::PruneOldSnapshots()
{
	while (Snapshots.Num() > MaxSnapshots)
	{
		...
		if (!OldestId.IsEmpty())
		{
			UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Pruning old snapshot '%s'"), *OldestId);
...
bool FUnrealAgentServer::SaveSnapshotToDisk(const FString& SnapshotId, const FGraphSnapshot& Snapshot)
{
	FString Dir = FPaths::ProjectSavedDir() / TEXT("UnrealAgent") / TEXT("Snapshots");
	...
	if (bSuccess)
	{
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Saved snapshot to disk: %s"), *FilePath);
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("UnrealAgent: Failed to save snapshot to disk: %s"), *FilePath);
	}
	return bSuccess;
}

bool FUnrealAgentServer::LoadSnapshotFromDisk(const FString& SnapshotId, FGraphSnapshot& OutSnapshot)
{
	FString Dir = FPaths::ProjectSavedDir() / TEXT("UnrealAgent") / TEXT("Snapshots");
	FString FilePath = Dir / (SnapshotId + TEXT(".json"));
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Loaded snapshot from disk: %s"), *FilePath);
	return true;
}
...
FString FUnrealAgentServer::HandleSnapshotGraph(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Creating snapshot for blueprint '%s'"), *BlueprintName);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Snapshot '%s' created with %d graphs, %d total connections"),
...
FString FUnrealAgentServer::HandleDiffGraph(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Diffing blueprint '%s' against snapshot '%s'"), *BlueprintName, *SnapshotId);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Diff complete — %d severed, %d new, %d type changes, %d missing nodes"),
...
FString FUnrealAgentServer::HandleRestoreGraph(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Restoring connections from snapshot '%s' for blueprint '%s' (dryRun=%s)"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Restore complete — %d reconnected, %d failed, saved=%s"),
...
FString FUnrealAgentServer::HandleFindDisconnectedPins(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Finding disconnected pins across %d blueprint(s)"), BlueprintsToScan.Num());
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: FindDisconnectedPins complete — %d HIGH, %d MEDIUM, %d total across %d blueprints"),
...
FString FUnrealAgentServer::HandleAnalyzeRebuildImpact(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Analyzing rebuild impact for module '%s'"), *ModuleName);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Found %d structs and %d enums in module '%s'"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Rebuild impact analysis complete — %d affected blueprints, %d Break/Make nodes, %d connections at risk"),
