...
FString FUnrealAgentServer::HandleSetViewMode(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_view_mode('%s')"), *ModeName);
...
FString FUnrealAgentServer::HandleSetShowFlags(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_show_flags('%s', %s)"), *FlagName, bEnabled ? TEXT("true") : TEXT("false"));
...
FString FUnrealAgentServer::HandleSetViewportType(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_viewport_type('%s')"), *TypeName);
...
FString FUnrealAgentServer::HandleSetRealtimeRendering(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_realtime_rendering(%s)"), bEnabled ? TEXT("true") : TEXT("false"));
...
FString FUnrealAgentServer::HandleSetGameView(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_game_view(%s)"), bEnabled ? TEXT("true") : TEXT("false"));
