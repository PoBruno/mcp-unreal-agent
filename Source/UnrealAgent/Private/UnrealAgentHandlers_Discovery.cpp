...
FString FUnrealAgentServer::HandleGetPinInfo(const FString& Body)
{
...
FString FUnrealAgentServer::HandleCheckPinCompatibility(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleListClasses(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleListFunctions(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleListProperties(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
