...
FString FUnrealAgentServer::HandleAddEventDispatcher(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Adding event dispatcher '%s' to Blueprint '%s'"),
		*DispatcherName, *BlueprintName);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Added event dispatcher '%s' to '%s' with %d params (saved: %s)"),
...
FString FUnrealAgentServer::HandleListEventDispatchers(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
