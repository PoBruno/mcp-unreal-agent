...
// (Shared with UnrealAgentHandlers_LevelActors.cpp — uses same logic)
// ============================================================
...
FString FUnrealAgentServer::HandleSetActorMobility(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_actor_mobility('%s', '%s')"), *ActorLabel, *MobilityStr);
...
FString FUnrealAgentServer::HandleSetActorVisibility(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_actor_visibility('%s', visible=%s)"),
...
FString FUnrealAgentServer::HandleSetActorPhysics(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_actor_physics('%s', simulate=%s)"),
