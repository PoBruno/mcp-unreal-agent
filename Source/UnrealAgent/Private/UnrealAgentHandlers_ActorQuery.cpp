...
FString FUnrealAgentServer::HandleFindActorsByTag(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: find_actors_by_tag('%s')"), *Tag);
	FName TagName(*Tag);
...
FString FUnrealAgentServer::HandleFindActorsByClass(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body."));
	FString ClassName;
	...
	UWorld* World = GEditor->GetEditorWorldContext().World();
	if (!World) return MakeErrorJson(TEXT("No editor world available."));
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: find_actors_by_class('%s')"), *ClassName);
	TArray<TSharedPtr<FJsonValue>> Results;
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		AActor* Actor = *It;
...
FString FUnrealAgentServer::HandleFindActorsInRadius(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body."));
	...
	UWorld* World = GEditor->GetEditorWorldContext().World();
	if (!World) return MakeErrorJson(TEXT("No editor world available."));
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: find_actors_in_radius(%.0f, %.0f, %.0f, r=%.0f)"), X, Y, Z, Radius);
...
FString FUnrealAgentServer::HandleGetActorBounds(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body."));
	FString ActorLabel;
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: get_actor_bounds('%s')"), *ActorLabel);
...
FString FUnrealAgentServer::HandleSetActorTags(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body."));
	FString ActorLabel;
	...
	AActor* Actor = FindActorByLabelQuery(World, ActorLabel);
	if (!Actor) return MakeErrorJson(FString::Printf(TEXT("Actor '%s' not found."), *ActorLabel));
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: set_actor_tags('%s', %d tags)"), *ActorLabel, TagsArr->Num());
	Actor->Tags.Empty();
