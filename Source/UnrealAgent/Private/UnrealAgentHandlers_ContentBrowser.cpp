...
FString FUnrealAgentServer::HandleNavigateContentBrowser(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: navigate_content_browser('%s')"), *Path);
...
FString FUnrealAgentServer::HandleOpenAssetEditor(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: open_asset_editor('%s')"), *AssetPath);
