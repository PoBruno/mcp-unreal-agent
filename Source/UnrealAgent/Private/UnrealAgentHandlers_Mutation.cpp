...
// SEH wrapper defined in UnrealAgentServer.cpp — non-static for cross-TU access
#if PLATFORM_WINDOWS
...
FString FUnrealAgentServer::HandleReplaceFunctionCalls(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: %s function calls in '%s': %s -> %s (%s)"),
	...
	for (UK2Node_CallFunction* CallNode : AllCallNodes)
	{
		...
		if (!NewFunc)
		{
			UE_LOG(LogTemp, Warning, TEXT("UnrealAgent: Function '%s' not found in '%s', skipping node"),
	...
	if (ReplacedCount > 0)
	{
		...
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Replaced %d function call(s), save %s"),
...
FString FUnrealAgentServer::HandleDeleteAsset(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	if (bForce)
	{
		...
		if (Package)
		{
			UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Force-unloading package '%s' from memory"), *AssetPath);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Deleting asset '%s' (%s)%s"),
...
FString FUnrealAgentServer::HandleConnectPins(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Connecting %s.%s -> %s.%s"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Connection %s, save %s"),
...
FString FUnrealAgentServer::HandleDisconnectPin(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Disconnected %d link(s) from %s.%s"),
...
FString FUnrealAgentServer::HandleRefreshAllNodes(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Refreshing all nodes in '%s' (%d graphs, %d nodes)"),
	...
	if (RefreshResult != 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("UnrealAgent: RefreshAllNodes crashed (SEH), proceeding to save"));
	...
	{
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Removed %d orphaned pins"), OrphanedPinsRemoved);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: RefreshAllNodes complete, save %s"),
		bSaved ? TEXT("succeeded") : TEXT("failed"));
...
FString FUnrealAgentServer::HandleSetPinDefault(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	if (Json->TryGetArrayField(TEXT("batch"), BatchArray) && BatchArray && BatchArray->Num() > 0)
	{
		...
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Batch SetPinDefault — %d/%d succeeded, save %s"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: SetPinDefault on '%s' pin '%s': '%s' -> '%s'"),
...
FString FUnrealAgentServer::HandleChangeStructNodeType(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Changing struct node '%s' to type '%s'"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Saved %d pin connections to reconnect"), ConnectionsByBaseName.Num());
...
FString FUnrealAgentServer::HandleDeleteNode(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Deleting node '%s' (%s) from graph '%s' in '%s'"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Node deleted, save %s"),
		bSaved ? TEXT("succeeded") : TEXT("failed"));
...
FString FUnrealAgentServer::HandleAddNode(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
		...
		for (UEdGraphNode* ExistingNode : TargetGraph->Nodes)
		{
			if (UK2Node_Event* ExistingEvent = Cast<UK2Node_Event>(ExistingNode))
			{
					...
					ExistingEvent->EventReference.GetMemberName() == FName(*FunctionName))
				{
					...
					UE_LOG(LogTemp, Display, TEXT("UnrealAgent: OverrideEvent '%s' already exists in '%s', returning existing node"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Added %s node '%s' in graph '%s' of '%s', save %s"),
...
FString FUnrealAgentServer::HandleRenameAsset(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Renaming asset '%s' -> '%s'"), *AssetPath, *NewPath);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Rename %s"), bSuccess ? TEXT("succeeded") : TEXT("failed"));
...
FString FUnrealAgentServer::HandleMoveNode(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	if (bBatchMode)
	{
		...
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: MoveNode batch — %d/%d succeeded, save %s"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: MoveNode '%s' from (%d,%d) to (%d,%d)"),
...
FString FUnrealAgentServer::HandleDuplicateNodes(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Duplicating %d node(s) in graph '%s' of '%s'"),
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Duplicated %d node(s), save %s"),
...
FString FUnrealAgentServer::HandleGetNodeComment(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
...
FString FUnrealAgentServer::HandleSetNodeComment(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Set comment on node '%s' in '%s', save %s"),
