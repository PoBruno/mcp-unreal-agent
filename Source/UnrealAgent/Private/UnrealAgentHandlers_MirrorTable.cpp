...
FString FUnrealAgentServer::HandleListMirrorTableRows(const FString& Body)
{
...
FString FUnrealAgentServer::HandleSetMirrorTableRows(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) { return MakeErrorJson(TEXT("Invalid JSON body")); }
...
FString FUnrealAgentServer::HandleRemoveMirrorTableRows(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) { return MakeErrorJson(TEXT("Invalid JSON body")); }
	FString TableName;
