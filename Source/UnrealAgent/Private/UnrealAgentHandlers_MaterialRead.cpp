...
FString FUnrealAgentServer::HandleListMaterials(const TMap<FString, FString>& Params)
{
...
FString FUnrealAgentServer::HandleGetMaterial(const TMap<FString, FString>& Params)
{
	...
	if (Material)
	{
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: GetMaterial — loaded material '%s'"), *Material->GetName());
	...
	if (MI)
	{
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: GetMaterial — loaded material instance '%s'"), *MI->GetName());
...
FString FUnrealAgentServer::HandleGetMaterialGraph(const TMap<FString, FString>& Params)
{
	const FString* Name = Params.Find(TEXT("name"));
	if (!Name || Name->IsEmpty())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: GetMaterialGraph — material '%s'"), *Material->GetName());

	// Ensure the material graph is built
	if (!Material->MaterialGraph)
	{
		UE_LOG(LogTemp, Display, TEXT("UnrealAgent: GetMaterialGraph — MaterialGraph is null, attempting rebuild"));
...
FString FUnrealAgentServer::HandleDescribeMaterial(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: DescribeMaterial — '%s'"), *Material->GetName());
...
FString FUnrealAgentServer::HandleSearchMaterials(const TMap<FString, FString>& Params)
{
...
FString FUnrealAgentServer::HandleFindMaterialReferences(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: FindMaterialReferences — '%s' (package: %s)"), *MaterialName, *PackagePath);
...
FString FUnrealAgentServer::HandleListMaterialFunctions(const TMap<FString, FString>& Params)
{
	const FString* Filter = Params.Find(TEXT("filter"));
...
FString FUnrealAgentServer::HandleGetMaterialFunction(const TMap<FString, FString>& Params)
{
	const FString* Name = Params.Find(TEXT("name"));
	if (!Name || Name->IsEmpty())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: GetMaterialFunction — '%s'"), *MF->GetName());
...
FString FUnrealAgentServer::HandleValidateMaterial(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Validating material '%s'"), *Material->GetName());
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Material '%s' validation %s (%d errors)"),
