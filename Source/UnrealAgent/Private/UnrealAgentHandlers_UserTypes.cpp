...
FString FUnrealAgentServer::HandleCreateStruct(const FString& Body)
{
	...
	if (Json->TryGetArrayField(TEXT("properties"), PropsArray) && PropsArray)
	{
		for (const TSharedPtr<FJsonValue>& PropVal : *PropsArray)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UnrealAgent: Could not resolve type '%s' for property '%s': %s"), *PropType, *PropName, *TypeError);
				continue;
			}
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Created UserDefinedStruct '%s' with %d properties, save %s"),
...
FString FUnrealAgentServer::HandleCreateEnum(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Created UserDefinedEnum '%s' with %d values, save %s"),
...
FString FUnrealAgentServer::HandleAddStructProperty(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Added property '%s' (%s) to struct '%s', save %s"),
...
FString FUnrealAgentServer::HandleRemoveStructProperty(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid())
	{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Removed property '%s' from struct '%s', save %s"),
