...
public:
	...
	virtual void Serialize(const TCHAR* V, ELogVerbosity::Type Verbosity, const FName& Category) override
	{
		...
		{
			if (!Msg.Contains(TEXT("UnrealAgent:")))
			{
...
FString FUnrealAgentServer::HandleValidateBlueprint(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Validating blueprint '%s'"), *BlueprintName);
...
FString FUnrealAgentServer::HandleValidateAllBlueprints(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	...
	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Bulk validating blueprints (filter: '%s', range: %d-%d of %d matching)"),
	...
	for (int32 Idx = StartIdx; Idx < EndIdx; Idx++)
	{
		...
		{
			UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Validated %d blueprints so far (%d failed)..."),
				TotalChecked, TotalFailed);
		}
	}

	UE_LOG(LogTemp, Display, TEXT("UnrealAgent: Bulk validation complete — %d checked, %d passed, %d failed, %d crashed"),
